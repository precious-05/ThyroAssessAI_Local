// ============================================
// HISTORY.JS - COMPLETELY FIXED VERSION
// ============================================
...
(function() {
    ...
    function updateHeaderStats(stats) {
        ...
        if (totalElement) totalElement.textContent = stats.total_predictions || 0;
        if (avgRiskElement) avgRiskElement.textContent = `${stats.risk_stats?.average || 0}%`;
        if (lastUpdatedElement) lastUpdatedElement.textContent = stats.last_updated || '--';
    }
    ...
    function calculateHeaderStats() {
        ...
        if (totalElement) totalElement.textContent = historyData.length;
        if (avgRiskElement) {
    ...
    async function loadHistory() {
        try {
            ...
            const response = await fetch(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.HISTORY}?limit=100`);
    ...
    function processHistoryData() {
        filteredHistory = [...historyData].sort((a, b) => {
            try {
                ...
                return bValue - aValue; // Latest first
            } catch (error) {
                return 0;
            }
        });
        ...
        if (distributionChart) {
            distributionChart.innerHTML = `
                <div class="chart-placeholder">
                    ...
                    <p>Risk distribution chart will appear here</p>
                </div>
            `;
        }
        ...
        if (trendChart) {
            trendChart.innerHTML = `
                <div class="chart-placeholder">
                    ...
                    <p>Risk trend chart will appear here</p>
                </div>
            `;
        }
    }
    ...
    // ✅ FIXED: Update List View - NO DELETE BUTTONS
    ...
    // ✅ FIXED: Create History Row - ONLY VIEW DETAILS BUTTON
    function createHistoryRow(item, index) {
        ...
        try {
            ...
            // ✅ REMOVED: Delete button from actions
            // ✅ KEPT: Only View Details and Download buttons
            row.innerHTML = `
    ...
    // ✅ FIXED: Update Grid View - NO DELETE BUTTONS
    ...
    // ✅ FIXED: Create History Card - ONLY VIEW DETAILS BUTTON
    function createHistoryCard(item, index) {
        ...
        try {
            ...
            // ✅ FIXED: Event handlers without delete button
            card.querySelector('.btn-view-full')?.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                showHistoryDetails(item);
            });
    ...
    // ✅ FIXED: Setup History Event Listeners - CLEAR ALL FIX
    function setupHistoryEventListeners() {
        ...
        // ✅ FIXED: Clear button - Simplified event handler
    ...
    // ✅ FIXED: Show History Details - NO FREEZE
    function showHistoryDetails(item) {
        ...
        try {
            ...
            // ✅ FIXED: Add overlay click handler properly
            ...
            console.error('Error showing history details:', error);
    ...
    function populateDetailModal(item) {
        try {
            ...
            if (riskPercentageElement) riskPercentageElement.textContent = `${riskPercentage}%`;
                ...
                if (activeFactors.length > 0) {
                    factorsList.innerHTML = activeFactors.map(factor => `
                        <div class="parameter-item">
                            ...
                            <span class="param-value"><i class="fas fa-check-circle"></i></span>
                        </div>
                    `).join('');
                } else {
    ...
    // ✅ FIXED: Show Confirm Modal - PROPERLY
    ...
    // ✅ FIXED: Close Confirm Modal - PROPERLY
